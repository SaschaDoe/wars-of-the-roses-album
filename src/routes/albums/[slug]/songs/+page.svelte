...
<svelte:head>
	<title>{$currentTranslations.nav.songs} - {albumTitle}</title>
</svelte:head>
...
<div class="songs-page" class:visible>
	...
	<div class="album-info">
		<div class="album-cover-small">
			<img src={album.coverImage} alt={albumTitle} />
		</div>
		<div class="album-details">
			<h2>Uncivil War</h2>
			<p class="release-info">{album.year} • {songs.length} {$currentTranslations.songs.tracks}</p>
	...
	<div class="tracklist">
		{#each songs as song, index}
			<div class="track-item" style="animation-delay: {index * 0.1}s" class:is-playing={isPlaying(song)}>
				...
				<a href="/albums/{album.slug}/songs/{song.id}" class="track-info">
					<h3 class="track-title">{song.title}</h3>
					<p class="track-context">{song.historicalContext.year} • {song.historicalContext.location?.name || 'Various'}</p>
				</a>
				<div class="track-duration">{song.duration}</div>
				<a href="/albums/{album.slug}/songs/{song.id}" class="track-link-icon" aria-label="View song details">
